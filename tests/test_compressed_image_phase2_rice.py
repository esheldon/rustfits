...
# -------------------- non-RICE compression rejected ----------------


@pytest.mark.parametrize("compress", ["GZIP_1", "GZIP_2", "PLIO_1"])
def test_non_rice_read_rejects(compress):
    with tempfile.TemporaryDirectory() as tmpdir:
        fname = os.path.join(tmpdir, "t.fits.fz")
        ...
        with rustfits.FITS(fname, "r") as fits:
            with pytest.raises(
                NotImplementedError, match=compress,
            ):
                fits[1].read()
